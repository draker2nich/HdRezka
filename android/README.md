...
пульт: пауза, перемотка ±15 c, громкость, стоп, смена серии/озвучки.
...
- Авторизация в аккаунте HDRezka не реализована (премиум-озвучки и контент,
  требующий логина, будут недоступны) — при необходимости добавить экран логина
  по образцу `login()` из `HdRezkaApi/api.py`.
- Токен-авторизация агента (см. раздел «Безопасность v2» в `AGENT_API.md`) пока
  не реализована ни на телефоне, ни на приставке.
- Пагинация каталога/поиска: грузится первая страница (методы `categoryPage`/
  `searchPage` в `HdRezkaSearch.kt` уже принимают номер страницы — осталось
  подключить подгрузку при скролле).
