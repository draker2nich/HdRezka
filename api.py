...
class HdRezkaApi(object):
	...
	def sort_translators(self, translators=None, priority=None, non_priority=None):
		...
		# БАГФИКС: раньше тут было `dict(sorted(...))`. Обычный dict в
		# Python 2.7 НЕ сохраняет порядок вставки — то есть результат
		# sorted() терялся сразу же при упаковке обратно в dict, и весь
		# смысл приоритета переводов (какая озвучка показывается первой)
		# фактически не работал предсказуемо. OrderedDict сохраняет
		# порядок, как и задумывалось изначально.
		sorted_translators = OrderedDict(sorted((translators or self.translators).items(),
