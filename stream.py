...
class HdRezkaStream(object):
	...
	def __call__(self, resolution):
		...
		# БАГФИКС: раньше брался coincidences[0] из py2-dict без порядка —
		# при нескольких совпадениях ("1080p" и "1080p Ultra") результат
		# был недетерминированным. Теперь: точное совпадение приоритетно,
		# иначе кратчайшее из совпадений.
		if target in self._videos:
			return self._videos[target]
		coincidences = sorted((x for x in self._videos if target in x), key=len)
		if coincidences:
	...
	def __init__(self, data, codes):
		...
		if data:
			...
			for i in arr:
				...
				lang = temp[0]
				link = temp[1]
				# .get: сервер иногда отдаёт язык без кода — раньше KeyError
				# ронял весь getStream из-за одной битой дорожки субтитров.
				code = codes.get(lang, lang) if hasattr(codes, "get") else codes[lang]
